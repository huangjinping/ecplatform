...
public class EUITreeUtil {
	...
	/** 树根节点ID */
	public static final Long TREE_ROOT_ID = new Long(0);
	...
	public static final <T extends BaseEUITreeEntity> boolean 
		hasParent(BaseEUITreeEntity entity, List<T> treeNodeList) {
    	for (BaseEUITreeEntity e : treeNodeList) {
    		if (entity.getParentId() == e.getId()) {
    			return true;
    		}
    	}
    	return false;
	...
	public static final <T extends BaseEUITreeEntity> String 
		buildTreeJson(List<T> nodesList, Long rootNodeId) {
		return buildTreeJson(nodesList, rootNodeId, false);
	}
	
	public static final <T extends BaseEUITreeEntity> String 
		buildTreeJson(List<T> nodesList, Long rootNodeId, boolean checkable) {
		if (nodesList == null || nodesList.size() == 0) {
			return getEmptyTreeJson();
		}
		StringBuilder treeJson = new StringBuilder();
		Map<Long, List<T>> nodeMap = listToNodesMap(nodesList);
	...
	public static final <T extends BaseEUITreeEntity> String 
		buildTreeJson(Map<Long, List<T>> nodeMap) {
		return buildTreeJson(nodeMap, TREE_ROOT_ID, false);
	}
	
	public static final <T extends BaseEUITreeEntity> String 
		buildTreeJson(Map<Long, List<T>> nodeMap, boolean checkable) {
		return buildTreeJson(nodeMap, TREE_ROOT_ID, checkable);
	}
	
	public static final <T extends BaseEUITreeEntity> String 
		buildTreeJson(Map<Long, List<T>> nodeMap, Long rootNodeId, boolean checkable) {
		if (nodeMap == null || nodeMap.values().size() == 0) {
			return getEmptyTreeJson();
		}
		StringBuilder treeJson = new StringBuilder();
		List<T> rootNodes = nodeMap.get(rootNodeId);
	 ...
	 * @return
	 */
	public static final <T extends BaseEUITreeEntity> Map<Long, List<T>> 
	...
	public static final <T extends BaseEUITreeEntity> 
			void buildChildNodesJson(Map<Long, List<T>> nodeMap, 
...
//		}
	...
	void buildNodeFields(T t, StringBuilder treeJson, 
		...
		try {
			...
			if (hasBaseProperty) {
				treeJson.append("\"createPerson\":\"")
					.append(null2Empty(t.getCreateBy()))
					.append("\",")
					.append("\"createTime\":\"")
					.append(null2Empty(t.getCreateTime()))
					.append("\",")
					.append("\"updatePerson\":\"")
					.append(null2Empty(t.getUpdateBy()))
					.append("\",")
					.append("\"updateTime\":\"")
					.append(null2Empty(t.getUpdateTime()))
					.append("\",");
